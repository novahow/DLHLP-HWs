...
from miniasr.module.scheduler import create_lambda_lr_warmup
...
class BaseRNNT(pl.LightningModule):
    '''
    ...
    def configure_optimizers(self):
        '''opt = getattr(torch.optim, self.args.model.optim.algo)(
            self.parameters(), **self.args.model.optim.kwargs)
        sched = create_lambda_lr_warmup(opt)
        return [opt]#, [sched]'''
        ''' Sets optimizer. 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            # Decode (hypotheses)
            hyps = self.decode(feat, feat_len)
