...
class XNLIDs(Dataset):
    def __init__(self, fname, dev=True):
        self.lang = list()
        self.label = list()
        self.premise = list()
        self.hypothesis = list()
        for i, line in enumerate(open(fname).readlines()):
            ...
            self.premise.append(line[6 if dev else 2])
            self.hypothesis.append(line[7 if dev else 3])
...
def test_xnli(args):
    ...
    infer_dataset = XNLIDs('/work/dlhlp2022/HW5/XNLI-1.0/xnli.test.tsv', dev=False)
        ...
        for lang in langs[:]:
            ...
            for n in shot:
                ...
                for i, e in enumerate(tqdm(infer_dataset)):
                    ...
                    max_len_b = 0
                    d = [0] * 2
                    hints = [transp[:] + e[p] + '=>' for p in range(len(e) - 1)]
                    max_len_b = max([len(e[p].split()) * 1.2 + 100 for p in range(len(e) - 1)])
                    
                    res = model.lm.translate(hints, beam=1, max_len_a=1.0,  max_len_b=max_len_b, replace_newlines_with_eos=True)
                    d = [p.split('=>')[-1] for p in res]
                    if not i:
                        print(samps)
                        print(d)
                    for s in samps:
                        prefix += (s + '\n')
                    predict = model.XNLI_eval(prefix + d[0], d[1], val_dataset.label2prompt)
                    if predict == e[2]:
                        acc += 1.0
